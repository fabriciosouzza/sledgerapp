...
export function AccountStrip({ accounts, cards, period }: { accounts: AccountTile[]; cards: CardView[]; period: string }) {
  ...
  const tiles = [
    ...accounts.map((a) => ({ key: a.account.id, known: a.balanceCents !== null, node: <CashTile account={a} period={period} /> })),
    ...cards.map((c) => ({
      key: c.account.id,
...
function CashTile({ account: { account, balanceCents }, period }: { account: AccountTile; period: string }) {
  return (
    <Link
