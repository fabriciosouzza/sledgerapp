...
export function RecordBatchForm({ assets, today }: { assets: { id: string; name: string; balanceCents: number }[]; today: string }) {
  ...
  const [kind, setKind] = useState<Kind>("yield");
    ...
    <form onSubmit={onSubmit} className="space-y-5">
              ...
              <label htmlFor={`amount-${a.id}`} className="min-w-0">
                ...
                <span className={cn("block text-xs tabular-nums text-muted-foreground", d !== null && d < 0 && "text-red-600 dark:text-red-400")}>
                  {d === null || mode === "amount"
                    ...
                    : d === 0
                      ? "same as recorded"
                      : `${d > 0 ? "+" : "−"}${formatBRL(Math.abs(d))} vs ${formatBRL(a.balanceCents)}`}
                </span>
              </label>
      ...
      {kind === "yield" && negatives > 0 && (
        <p className="text-sm text-red-600 dark:text-red-400">
          {negatives} {negatives === 1 ? "line goes" : "lines go"} down: record those as a market adjustment.
        </p>
      )}

      <Button type="submit" size="lg" className="h-12 w-full text-base" disabled={pending || filled.length === 0 || (kind === "yield" && negatives > 0)}>
