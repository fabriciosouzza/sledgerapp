...
| 1.3 | **Fewer queries on Today** | Today runs ~10 queries (cards in parallel, recurrences, planned rows, movements, the rolling rate when the month says little). Fine per request; it adds up on a slow phone connection. | Cache the read-only overview per user for a short window with `unstable_cache` keyed by `userId`, invalidated by the write actions (they already call `revalidatePath`). Measure first: the Vercel function log shows the per-route time. |
...
| 3.3 | **`summary.ts` and `today.ts`** | Month deltas "through the same day", the six-month history against today's caps, `dueSoonCents`, `toReceive`, `dueToday` have no direct test. | Service tests with the fake repositories, the way `cards.test.ts` does it. |
